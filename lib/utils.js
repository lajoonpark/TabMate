...
/**
 * Finds groups of tabs sharing the same duplicate key.
 * Only groups with two or more tabs are returned.
 *
 * @param {chrome.tabs.Tab[]} tabs
 * @param {{ mode?: 'exact' | 'generalized', ignoreHash?: boolean, ignoreQuery?: boolean }} [options]
 * @returns {{ key: string, label: string, tabs: chrome.tabs.Tab[] }[]}
 */
export function findDuplicateGroups(
  tabs,
  { mode = 'exact', ignoreHash = true, ignoreQuery = false } = {}
) {
  ...
  tabs.forEach((tab) => {
    ...
    const key =
      mode === 'generalized'
        ...
        : normalizeUrl(tab.url, { ignoreHash, ignoreQuery });
    const label = mode === 'generalized' ? parsed.hostname : key;

    if (!groups.has(key)) groups.set(key, { key, label, tabs: [] });
    groups.get(key).tabs.push(tab);
  });

  return [...groups.values()].filter((group) => group.tabs.length > 1);
}
...
/**
 * Returns the duplicate tabs that can be safely closed while preserving
 * the preferred copy and any protected pinned tabs.
 *
 * @param {chrome.tabs.Tab[]} tabs
 * @param {{ closePinnedTabs?: boolean }} [options]
 * @returns {{ keep: chrome.tabs.Tab | null, tabsToClose: chrome.tabs.Tab[] }}
 */
export function planDuplicateTabRemoval(tabs, { closePinnedTabs = false } = {}) {
  const keep = selectPreferredTab(tabs);
  if (!keep) return { keep: null, tabsToClose: [] };

  return {
    keep,
    tabsToClose: tabs.filter((tab) => {
      if (tab.id === keep.id || !Number.isInteger(tab.id)) return false;
      if (tab.active) return false;
      if (!closePinnedTabs && tab.pinned) return false;
      return true;
    }),
  };
}
