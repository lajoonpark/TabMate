 ...
 * @param {{ responseFormat?: boolean, maxTokens?: number, requireContent?: boolean }} [options]
 * @returns {Promise<{ ok: boolean, content?: string, code?: string, error?: string }>}
 */
async function postChatCompletion(apiKey, model, messages, { responseFormat = true, maxTokens = 0, requireContent = true } = {}) {
  ...
  try {
    ...
    // Connection tests use max_tokens=1. Reasoning models often spend that
    // budget on thinking and return content=null with HTTP 200 / finish_reason=length.
    if (!requireContent && json?.choices?.[0]) return { ok: true, content: '' };
