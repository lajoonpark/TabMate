...
async function postChatCompletion(apiKey, model, messages, { responseFormat = true, maxTokens = 0 } = {}) {
  ...
  try {
    ...
    if (typeof content !== 'string') {
      // Diagnostic: Test uses max_tokens=1. Reasoning models often spend that
      // budget on thinking and return content=null with HTTP 200.
      console.warn('[TabMate] OpenRouter unexpected response', {
        model,
        maxTokens,
        responseFormat,
        choiceCount: Array.isArray(json?.choices) ? json.choices.length : null,
