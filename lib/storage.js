...
/**
 * Fills in any missing fields (colour, priority, rules, undeletable) on a
 * stored category by looking up its counterpart in DEFAULT_CATEGORIES.
 * User customisations (name, colour overrides, custom rules) are preserved.
 *
 * @param {object} cat
 * @returns {import('./defaults.js').Category}
 */
function normalizeCategory(cat) {
  const def = DEFAULT_CATEGORIES.find((d) => d.id === cat.id);
  return {
    ...cat,
    colour: cat.colour ?? def?.colour ?? 'grey',
    priority: cat.priority ?? def?.priority ?? 500,
    rules: cat.rules ?? def?.rules ?? [],
    // "Other" is always undeletable regardless of what was stored
    undeletable: cat.id === 'other' ? true : (cat.undeletable ?? def?.undeletable ?? false),
  };
}
...
export async function getCategories() {
  ...
  if (!Array.isArray(stored)) return DEFAULT_CATEGORIES.map(normalizeCategory);
...
export async function initDefaults() {
  ...
  if (!syncData.customCategories) syncUpdates.customCategories = DEFAULT_CATEGORIES.map((c) => ({ ...c }));
